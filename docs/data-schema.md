...
Notion タスクDB (各proj) ────┴──▶ プロジェクトに集約 ────▶ │   + tasks[] ネスト        │
                                                          ├──────────────────────────┤
Notion 議事録DB (各proj) ───────▶ 全文そのまま保存 ────────▶ │ meetings     /meeting_id  │
                                                          ├──────────────────────────┤
...
### 4.3 meetings（議事録 / 全文保存）

1議事録 = 1ドキュメント。チャンク分割・ベクトル埋め込み・Ingest時のLLM分析は行わない。
`full_text`（原文）をそのまま保存し、分析はエージェントが必要なときに行う。
...
```json
{
  ...
  "source": { "notion_page_id": "<meeting-page-id>", "synced_at": "2026-05-27T12:00:00Z" }
}
```

- 1会議 = 1ドキュメント。`id` は Notion 議事録ページの ID。
- エージェントは `project_id` / `participants` でフィルターして `full_text` を読み、必要に応じて分析する。
...
| タスクDB: タスクページID | → | tasks[].task_id | そのまま |
...
| タスクページ本文ブロック | → | tasks[].description | ブロックをMarkdown文字列に変換して保存 |
...
| 参加者 | → | participants / participant_names | 氏名→email解決して配列化 / 氏名配列 |
| 本文 | → | full_text | rich_textを結合してそのまま保存 |
...
## 8. RAG / ベクトル検索（廃止済み）・Ingest時LLM分析（廃止済み）

- **方針確定**: チャンク分割・ベクトル埋め込みは採用しない。
- **Ingest時LLM分析も廃止**: 以前は Ingest 時に gpt-4o で議事録の `overall_summary` / `member_analyses[]` を生成していたが廃止。
- **現方針**: 議事録は `full_text`（原文）をそのまま保存。分析はエージェントが必要なときに行う（§4.3）。
- **理由**: Ingest をシンプル・安く保ち、分析の質はエージェントに任せる。デモ規模（議事録9件）ではフィルター検索で十分。
